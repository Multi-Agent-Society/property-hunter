...
def generate_article(topic):
    ...
    return PropertyHunterCrew().crew().kickoff(inputs=inputs)


def display_sidebar_ui():
    with st.sidebar:
        st.title('Property Hunter')
...
#         abort_chat(f"Conversation length too long. Please keep it under {max_tokens} tokens.")
    ...
    # for event_index, event in enumerate(chat_completion.choices[0].message.content):
